...
export function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative isolate flex h-svh min-h-[640px] items-start overflow-hidden bg-navy pt-28 text-cream md:items-center md:pt-0"
    >
      ...
      <motion.div
        ...
        className="neon-once pointer-events-none absolute inset-x-0 bottom-[-10vw] z-[25] flex h-[44vw] select-none items-center justify-center md:inset-x-auto md:bottom-auto md:left-[60.2vw] md:top-[-37.7vw] md:z-10 md:h-[126.7vw] md:w-[37.6vw]"
      >
        <div className="flex h-[44vw] w-full flex-none items-center justify-center whitespace-nowrap font-display text-[40vw] leading-none tracking-[-0.07em] text-transparent [-webkit-text-stroke:2px_rgb(245_240_230/0.7)] md:h-[37.6vw] md:w-[126.7vw] md:-rotate-90 md:justify-start md:text-[32.15vw] md:[-webkit-text-stroke:3px_rgb(245_240_230/0.55)]">
          wangeci
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute bottom-0 left-1/2 z-20 aspect-[1440/1024] h-[60%] -translate-x-[69.8%] md:left-auto md:right-0 md:h-full md:translate-x-0"
      >
        <Image
      ...
      {/* Text side */}
      <div className="relative z-30 pl-16 pr-6 md:pl-[14.2vw]">
        <motion.h1
          initial={{ opacity: 0, x: -60 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.9, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
          className="text-[clamp(7rem,15.56vw,18rem)] font-light leading-[0.92] tracking-[0.05em] text-gold"
        >
          Hello
        </motion.h1>
        <motion.p
          ...
          aria-live="polite"
          className="mt-6 flex flex-wrap items-baseline gap-x-2 text-[clamp(1rem,1.67vw,1.5rem)] tracking-[0.05em]"
        >
      ...
      <div
        aria-hidden
        className="absolute bottom-[24%] left-[21px] top-[32%] z-30 w-px -translate-x-1/2 bg-gold md:bottom-[15.5%] md:left-[7.6vw] md:top-[29%]"
      >
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 pb-3 text-xs leading-none tracking-[0.05em] text-white md:pb-5 md:text-[26px]">
          <AnimatePresence mode="wait" initial={false}>
            <motion.span
              key={role.label}
              initial={{ opacity: 0 }}
        ...
        <div className="absolute left-1/2 top-full -translate-x-1/2 pt-4 text-xs font-medium leading-none tracking-[0.05em] md:pt-7 md:text-[26px]">
      ...
      <div className="absolute bottom-[53%] left-1/2 z-30 flex -translate-x-1/2 items-center gap-3 md:bottom-[4%] md:left-[14.2vw] md:translate-x-0">
        {/* Text hidden on mobile: just the arrow, centred. */}
        <span className="hidden text-xl font-medium tracking-[0.07em] md:inline">Scroll down</span>
        {/* Mobile rests 5s between rounds; desktop keeps its 3s rest. */}
        <ScrollArrow rest={5} className="md:hidden" />
        <ScrollArrow rest={3} className="hidden md:block" />
      </div>
    </section>
  );
}
